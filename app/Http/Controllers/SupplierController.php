    ...
    public function stokbarang()
{
    ...
    $stokBarang = DB::table('t_stok')
        ->join('m_barang', 't_stok.barang_id', '=', 'm_barang.id')
        ->select(
            't_stok.id',
            'm_barang.nama_barang',
            't_stok.jumlah_masuk',
            't_stok.jumlah_keluar',
            DB::raw('(t_stok.jumlah_masuk - t_stok.jumlah_keluar) as sisa_stok'),
            't_stok.created_at'
        )
        ->get();

    return view('stokbarang.index', compact('activeMenu', 'stokBarang'));
}
}
